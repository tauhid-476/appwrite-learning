...
const Page = async ({
    ...
    questions.documents = await Promise.all(
        questions.documents.map(async ques => {
            ...
            let author;
            try {
                author = await users.get<UserPrefs>(ques.authorId);
            } catch (error) {
                console.error(`Failed to fetch user data for ID ${ques.authorId}:`, error);
                // Provide a default author object if we can't fetch the data
                author = {
                    $id: ques.authorId,
                    name: "Anonymous",
                    prefs: { reputation: 0 },
